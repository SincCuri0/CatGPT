...
export async function executeRuntimeSubagentsGet(req: NextRequest, debugEnabled: boolean): Promise<Response> {
    try {
...
export async function executeRuntimeSubagentsPost(req: NextRequest, debugEnabled: boolean): Promise<Response> {
    try {
        const access = authorizeRuntimeAccess(req);
        if (!access.ok) {
            debugRouteLog(debugEnabled, "api/runtime/subagents", "Rejected runtime access", { reason: access.reason });
